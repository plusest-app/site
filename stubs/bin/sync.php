 ...
 *
 *  Якщо CRM закриє доступ агентству назавжди, у службовому каталозі (paths.data)
 *  зʼявиться файл disallowed.lock, і скрипт завершуватиметься одразу, не
 *  турбуючи CRM. Коли доступ відновлять — видаліть цей файл.
 *
...
try {
    ...
    // --- Доступ до CRM закрито назавжди --------------------------------------
    //  Якщо CRM заблокувала агентство остаточно, у службовому каталозі лежить
    //  файл-позначка. Виходимо одразу: ні підключення до бази, ні запиту до
    //  CRM не буде.
    //
    //  Код виходу 0: це не збій прогону, а стабільний стан, і лист від cron
    //  щочверть години тут нікому не потрібен. Про саме блокування cron уже
    //  надіслав листа того разу, коли CRM про нього повідомила.
    $ban = Plusest\Site\Sync\Ban::fromConfig($config);

    if ($ban->exists()) {
        if (!$quiet) {
            $reason = $ban->reason();

            fwrite(STDOUT, 'Синхронізацію вимкнено: CRM закрила доступ агентству назавжди'
                . ($reason === null ? '' : ' (' . $reason . ')') . '.' . PHP_EOL
                . 'Щоб увімкнути її знову, видаліть файл ' . $ban->file() . PHP_EOL);
        }
